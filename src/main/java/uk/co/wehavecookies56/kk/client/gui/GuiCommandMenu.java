...
public class GuiCommandMenu extends GuiScreen {
	...
	public void drawCommandMenu (int width, int height) {
		...
		//System.out.println("Is KH1 Fire?: "+ExtendedPlayer.get(Minecraft.getMinecraft().player).getKH1Fire());
		float scale = 1.05f;
		int colour;
		// DRIVE
		GL11.glPushMatrix();
		...
		GL11.glPushMatrix();
		{
			...
			if(this.submenu == 0)
			{
				if (spells == null) {
					...
					//if (!ExtendedPlayer.get(mc.player).getRecharge() && !spells.isEmpty() && !ExtendedPlayer.get(mc.player).getDriveInUse().equals("Valor"))
