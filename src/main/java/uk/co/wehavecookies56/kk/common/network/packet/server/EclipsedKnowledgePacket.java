...
public class EclipsedKnowledgePacket extends AbstractMessage.AbstractServerMessage<EclipsedKnowledgePacket> {
    ...
    public void process (EntityPlayer player, Side side) {
        ...
        int rand = Utils.randomWithRange(0, Lists.Saix.size() - 1);
        player.inventory.addItemStackToInventory(new ItemStack(Lists.Saix.get(rand)));
        TextComponentTranslation manifestMessage = new TextComponentTranslation(Strings.Chat_Manifest, new TextComponentTranslation(Lists.Saix.get(rand).getUnlocalizedName() + ".name"));
