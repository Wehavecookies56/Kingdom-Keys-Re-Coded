...
public class EntityGummiShip extends Entity
{
	...
	private void updateMotion()
	{
		...
		/*if (this.previousStatus == EntityGummiShip.Status.IN_AIR && this.status != EntityGummiShip.Status.IN_AIR && this.status != EntityGummiShip.Status.ON_LAND)
		{
			this.waterLevel = this.getEntityBoundingBox().minY + (double)this.height;
			this.setPosition(this.posX, (double)(this.getWaterLevelAbove() - this.height) + 0.101D, this.posZ);
			this.motionY = 0.0D;
			this.lastYd = 0.0D;
			//this.status = EntityGummiShip.Status.IN_WATER;
		}
		else
		{
			if (this.status == EntityGummiShip.Status.IN_WATER)
			{
				d2 = (this.waterLevel - this.getEntityBoundingBox().minY) / (double)this.height;
				this.momentum = 0.9F;
			}
			else if (this.status == EntityGummiShip.Status.UNDER_FLOWING_WATER)
			{
				d1 = -7.0E-4D;
				this.momentum = 0.9F;
			}
			else if (this.status == EntityGummiShip.Status.UNDER_WATER)
			{
				d2 = 0.009999999776482582D;
				this.momentum = 0.45F;
			}
			else if (this.status == EntityGummiShip.Status.IN_AIR)
			{
				this.momentum = 0.9F;
			}
			else if (this.status == EntityGummiShip.Status.ON_LAND)
			{
