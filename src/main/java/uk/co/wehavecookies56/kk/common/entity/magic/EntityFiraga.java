...
public class EntityFiraga extends Entity {
	...
	public void onUpdate () {
		...
		if (!world.isRemote) PacketDispatcher.sendToAllAround(new SpawnFireParticles(this, 3), player, 64.0D);
		...
		if (!list.isEmpty()) for (int i = 0; i < list.size(); i++) {
			...
			e.setFire(5);
			e.attackEntityFrom(DamageSource.causePlayerDamage(player), DamageCalculation.getMagicDamage(player,3)*DamageCalculation.fireMultiplier);
		}
		aabb.expand(-2, -2, -2);
