...
public class EntityFire extends Entity {
	...
	public void onUpdate () {
		...
		if (!world.isRemote) PacketDispatcher.sendToAllAround(new SpawnFireParticles(this, 1), player, 64.0D);
		...
		if (!list.isEmpty()) list.forEach(e -> {
			e.setFire(5);
			e.attackEntityFrom(DamageSource.causePlayerDamage(player), DamageCalculation.getMagicDamage(player,1)*DamageCalculation.fireMultiplier);
		});
		aabb.expand(-2, -2, -2);
		super.onUpdate();
	}
