...
public class DepositMaterials extends AbstractMessage.AbstractServerMessage<DepositMaterials> {

	public DepositMaterials() {
	}
	...
	public void process(EntityPlayer player, Side side) {
		...
		/*
		 * for (int i = 0; i < 36; i++) if (player.inventory.mainInventory[i] != null)
		 * if (player.inventory.mainInventory[i].getItem() instanceof
		 * ItemSynthesisMaterial) { if
		 * (player.inventory.mainInventory[i].hasTagCompound()) { String s =
		 * player.inventory.mainInventory[i].getTagCompound().getString("material"); if
		 * (MaterialRegistry.isMaterialRegistered(s))
		 * player.getCapability(ModCapabilities.SYNTHESIS_MATERIALS,
		 * null).addMaterial(MaterialRegistry.get(s),
		 ...
		 * SyncMaterialData(player.getCapability(ModCapabilities.SYNTHESIS_MATERIALS,
