...
public class ContainerSynthesisBagM extends Container {
	...
	INV_START = InventorySynthesisBagM.INV_SIZE,
	...
	public ContainerSynthesisBagM(EntityPlayer player, InventoryPlayer invPlayer, InventorySynthesisBagM invBag)
	{
		...
		for(i = 0; i < 4; i++){
			for(j = 0; j < 7; j++){ 
				this.addSlotToContainer(new SlotSynthesisBag(invBag, j + i * 7, 26 + j * 18, 18 + i * 18));
			}
		}
		...
		if (slot != null && slot.getHasStack()) {
			...
			if (index < INV_START) {
				...
				if (itemstack1.getItem() instanceof ItemSynthesisMaterial) {
					if (!this.mergeItemStack(itemstack1, 0, InventorySynthesisBagM.INV_SIZE, false)) {
						return null;
					}
				}
				if (index >= INV_START)
