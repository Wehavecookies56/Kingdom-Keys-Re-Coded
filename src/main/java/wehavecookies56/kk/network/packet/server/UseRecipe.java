...
public class UseRecipe extends AbstractServerMessage<UseRecipe> {
	...
	public void process(EntityPlayer player, Side side) {
		...
		if (RecipeRegistry.get(recipe1) == null) {
			String message = "ERROR: Recipe for " + TextHelper.localize(recipe1 + ".name")
					+ " was not learnt because it is not a valid recipe, Report this to Wehavecookies56";
			...
			String message = "Recipe for " + TextHelper.localize(recipe1 + ".name")
					+ " was not learnt because you have already learnt it";
		...
		if (RecipeRegistry.get(recipe2) == null) {
			String message = "ERROR: Recipe for " + TextHelper.localize(recipe2 + ".name")
					+ " was not learnt because it is not a valid recipe, Report this to Wehavecookies56";
			TextHelper.sendFormattedChatMessage(message, EnumChatFormatting.RED, player);
		} else if (RecipeRegistry.isRecipeKnown(player, recipe2)) {
			String message = "Recipe for " + TextHelper.localize(recipe2 + ".name")
					+ " was not learnt because you have already learnt it";
			TextHelper.sendFormattedChatMessage(message, EnumChatFormatting.YELLOW, player);
		} else {
		...
		if (RecipeRegistry.get(recipe3) == null) {
			String message = "ERROR: Recipe for " + TextHelper.localize(recipe3 + ".name")
					+ " was not learnt because it is not a valid recipe, Report this to Wehavecookies56";
			TextHelper.sendFormattedChatMessage(message, EnumChatFormatting.RED, player);
		} else if (RecipeRegistry.isRecipeKnown(player, recipe3)) {
			String message = "Recipe for " + TextHelper.localize(recipe3 + ".name")
					+ " was not learnt because you have already learnt it";
			TextHelper.sendFormattedChatMessage(message, EnumChatFormatting.YELLOW, player);
		} else {
