				...
				if (player.inventory.mainInventory[i].getItem() instanceof ItemSynthesisMaterial) {
					if (player.inventory.mainInventory[i].hasTagCompound()) {
						...
						if (MaterialRegistry.isMaterialRegistered(s))
							ExtendedPlayerMaterials.get(player).addMaterial(MaterialRegistry.get(s),
									player.inventory.mainInventory[i].stackSize);
						player.inventory.setInventorySlotContents(i, null);
					}

				} else if (MaterialRegistry.isMaterialRegistered(
						player.inventory.mainInventory[i].getItem().getUnlocalizedName().toString())) {
					ExtendedPlayerMaterials.get(player).addMaterial(
							MaterialRegistry.get(player.inventory.mainInventory[i].getItem().getUnlocalizedName()),
							player.inventory.mainInventory[i].stackSize);
					player.inventory.setInventorySlotContents(i, null);
				}
	}
