...
public class TakeMaterials extends AbstractServerMessage<TakeMaterials> {
	...
	public void process(EntityPlayer player, Side side) {
		...
		if (materialName.startsWith("sm.")) {
			...
			else if (materialName.endsWith("gem") || materialName.equals(Strings.SM_ManifestIllusion)
					|| materialName.equals(Strings.SM_Orichalcum))
				material.getTagCompound().setString("rank", "A");
			else if (materialName.endsWith("crystal") || materialName.equals(Strings.SM_LostIllusion)
					|| materialName.equals(Strings.SM_OrichalcumPlus))
			...
			if (GameRegistry.findItem(Reference.MODID, materialName.replace("item.", "")) != null) {
				player.inventory.addItemStackToInventory(new ItemStack(
						GameRegistry.findItem(Reference.MODID, materialName.replace("item.", "")), amount));
				ExtendedPlayerMaterials.get(player).removeMaterial(MaterialRegistry.get(materialName), amount);
			} else {
				// VANILLA ITEMS HERE
			...
			if (GameRegistry.findBlock(Reference.MODID, materialName.replace("tile.", "")) != null) {
				player.inventory.addItemStackToInventory(new ItemStack(
						GameRegistry.findBlock(Reference.MODID, materialName.replace("tile.", "")), amount));
				ExtendedPlayerMaterials.get(player).removeMaterial(MaterialRegistry.get(materialName), amount);
			} else {
				// VANILLA BLOCKS HERE
			}
