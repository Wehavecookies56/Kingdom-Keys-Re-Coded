	...
	public void clientTick(ClientTickEvent event) {
		...
		if (event.phase == Phase.END)
			if (!(Minecraft.getMinecraft().currentScreen instanceof GuiMainMenu
					...
					|| Minecraft.getMinecraft().currentScreen instanceof GuiScreenServerList
				...
				SoundManager soundManager = ReflectionHelper.getPrivateValue(SoundHandler.class,
						Minecraft.getMinecraft().getSoundHandler(), "sndManager", "field_147694_f");
				Map playingSounds = ReflectionHelper.getPrivateValue(SoundManager.class, soundManager, "playingSounds",
						"field_148629_h");
				...
				while (iter.hasNext()) {
					...
					if (playingSounds.get(o) instanceof PositionedSound) {
						...
						this.resLoc = ReflectionHelper.getPrivateValue(PositionedSound.class, this.posSound,
								"positionedSoundLocation", "field_147664_a");
					}
				}

				if ("music.game".equals(resLoc.getResourcePath())
						|| "music.game.creative".equals(resLoc.getResourcePath())) {
				...
				if (!EventHandler.isHostiles) {
					 ...
					 * Music.lazyAfternoons); for(int i = 0; i <
					 * this.played.length; i++){ this.played[i] = false; }
					 * //this.played[0] = true; } }
					 */

					/*
					 * if(this.day && !this.raining) { if(this.rand.nextInt(2)
					 * == 0 && !this.played[0]) {
					 * Minecraft.getMinecraft().getSoundHandler().playSound(
					 * Sounds.posDay1); for(int i = 0; i < this.played.length;
					 * i++) { this.played[i] = false; } this.played[0] = true; }
					 * else if(!this.played[1]) {
					 * Minecraft.getMinecraft().getSoundHandler().playSound(
					 * Sounds.posDay2); for(int i = 0; i < this.played.length;
					 * i++) { this.played[i] = false; } this.played[1] = true; }
					 ...
					 * } //Raining if(this.raining) { if(this.rand.nextInt(2) ==
					 * 0 && !this.played[4]) {
					 * Minecraft.getMinecraft().getSoundHandler().playSound(
					...
					if (!Minecraft.getMinecraft().getSoundHandler().isSoundPlaying(Music.sinisterSundown)
							&& !Music.sinisterSundown.getSoundLocation().equals(this.resLoc)
							&& !"music.game".equals(this.resLoc.getResourcePath())
							&& !"music.game.creative".equals(this.resLoc.getResourcePath()))
				...
				while (iter.hasNext()) {
					Object o = iter.next();
					if (playingSounds.get(o) instanceof PositionedSound) {
						if (((PositionedSound) playingSounds.get(o)).getSoundLocation().getResourcePath()
								.equals("music.menu"))
							Minecraft.getMinecraft().getSoundHandler()
									.stopSound(((PositionedSound) playingSounds.get(o)));
						this.posSound = (PositionedSound) playingSounds.get(o);
						this.resLoc = ReflectionHelper.getPrivateValue(PositionedSound.class, this.posSound,
								"positionedSoundLocation", "field_147664_a");
					}
