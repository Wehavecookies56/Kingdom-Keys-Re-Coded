...
public class CommandLearnRecipe implements ICommand {
	...
	public void processCommand(ICommandSender sender, String[] args) throws CommandException {
		if (sender.getCommandSenderEntity() instanceof EntityPlayer)
			if (args.length == 0)
				TextHelper.sendFormattedChatMessage("Invalid arguments, usage \"/learnrecipe <name>\"",
						EnumChatFormatting.RED, (EntityPlayer) sender.getCommandSenderEntity());
			else if (RecipeRegistry.isRecipeKnown((EntityPlayer) sender.getCommandSenderEntity(),
					"item." + args[0].toLowerCase()))
				TextHelper.sendFormattedChatMessage("You already know this recipe", EnumChatFormatting.YELLOW,
						(EntityPlayer) sender.getCommandSenderEntity());
			else if (RecipeRegistry.isRecipeRegistered("item." + args[0].toLowerCase())) {
				RecipeRegistry.learnrecipe((EntityPlayer) sender.getCommandSenderEntity(),
						"item." + args[0].toLowerCase());
				TextHelper.sendFormattedChatMessage(
						"Successfully learnt recipe for "
								+ TextHelper.localize("item." + args[0].toLowerCase() + ".name"),
						EnumChatFormatting.GREEN, (EntityPlayer) sender.getCommandSenderEntity());
			} else if (args[0].equals("all")) {
				for (Object value : RecipeRegistry.getRecipeMap().values())
					if (value instanceof Recipe)
						if (!RecipeRegistry.isRecipeKnown((EntityPlayer) sender.getCommandSenderEntity(),
								((Recipe) value).getName()))
							RecipeRegistry.learnrecipe((EntityPlayer) sender.getCommandSenderEntity(),
									((Recipe) value).getName());
				TextHelper.sendFormattedChatMessage("Successfully learnt all the recipes", EnumChatFormatting.GREEN,
						(EntityPlayer) sender.getCommandSenderEntity());
			} else
				TextHelper.sendFormattedChatMessage("This recipe doesn't exist", EnumChatFormatting.RED,
						(EntityPlayer) sender.getCommandSenderEntity());
	}
