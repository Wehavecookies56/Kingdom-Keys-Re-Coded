		...
		if (sender.getCommandSenderEntity() instanceof EntityPlayer) {
			...
			if (args.length == 0) {
				if (props.inventoryKeychain.getStackInSlot(0) != null) {
					...
					PacketDispatcher.sendToAllAround(new RemoveItemInSlot("keychain", 0),
							(EntityPlayer) sender.getCommandSenderEntity(), 1);

					if (props.isKeybladeSummoned())
						if (player.inventory.getCurrentItem() != null
								&& player.inventory.getCurrentItem().getItem() instanceof ItemKeyblade) // props.isKeybladeSummoned()
																										// ==
																										// true
					...
					TextHelper.sendFormattedChatMessage("Your keychain has been removed!", EnumChatFormatting.YELLOW,
							(EntityPlayer) sender.getCommandSenderEntity());
				} else
					TextHelper.sendFormattedChatMessage("The chain slot has no chain!", EnumChatFormatting.RED,
							(EntityPlayer) sender.getCommandSenderEntity());
			} else if (args.length == 1) {
				EntityPlayerMP playermp = args.length == 1 ? getPlayer(sender, args[0])
				...
				if (propsmp.inventoryKeychain.getStackInSlot(0) != null) {
					...
					if (propsmp.isKeybladeSummoned())
						if (playermp.inventory.getCurrentItem() != null
								&& playermp.inventory.getCurrentItem().getItem() instanceof ItemKeyblade)
					...
					TextHelper.sendFormattedChatMessage(args[0] + "'s keychain has been removed!",
							EnumChatFormatting.YELLOW, (EntityPlayer) sender.getCommandSenderEntity());
				} else
					TextHelper.sendFormattedChatMessage("The chain slot has no chain!", EnumChatFormatting.RED,
							(EntityPlayer) sender.getCommandSenderEntity());
			} else
				TextHelper.sendFormattedChatMessage("Invalid arguments! Usage: /removechain [player]",
