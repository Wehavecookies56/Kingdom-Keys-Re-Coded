...
public class InputHandler {
	...
	public boolean antiFormCheck() {
		...
		if (random * 100 < prob) {
			...
			Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX,
					...
					SoundHelper.Select, 1f, 1f, false);
	...
	public void commandDown() {
		...
		// InsideMagic
			...
			if (GuiCommandMenu.magicselected < ExtendedPlayer.spells.size() - 1) {
				GuiCommandMenu.magicselected++;
				GuiCommandMenu.submenu = GuiCommandMenu.SUB_MAGIC;
			} else if (GuiCommandMenu.magicselected >= ExtendedPlayer.spells.size() - 1)
	...
	public void commandEnter() {
		...
		case GuiCommandMenu.MAGIC:
			if (GuiCommandMenu.submenu == GuiCommandMenu.SUB_MAIN) {
				if (ExtendedPlayer.get(player).getRecharge() == false && (!ExtendedPlayer.spells.isEmpty()
						&& !ExtendedPlayer.get(player).getDriveInUse().equals("Valor"))) {
					GuiCommandMenu.magicselected = 0;
					GuiCommandMenu.submenu = GuiCommandMenu.SUB_MAGIC;
		...
		case GuiCommandMenu.DRIVE:
			if (GuiCommandMenu.submenu == GuiCommandMenu.SUB_MAIN) {
				if (ExtendedPlayer.get(player).getInDrive()) {// Revert
					if (ExtendedPlayer.get(player).getDriveInUse().equals("Anti")
							&& ExtendedPlayer.get(player).cheatMode == false) {
						GuiCommandMenu.selected = GuiCommandMenu.ATTACK;
						world.playSound(player.posX, player.posY, player.posZ, SoundHelper.Error, 2f, 1f, false);
					} else {
		...
		if (GuiCommandMenu.selected == GuiCommandMenu.MAGIC && GuiCommandMenu.submenu == GuiCommandMenu.SUB_MAGIC) {
			...
			} else if (!ExtendedPlayer.get(player).getRecharge()
					|| Constants.getCost(ExtendedPlayer.spells.get(GuiCommandMenu.magicselected)) == -1
							&& ExtendedPlayer.get(player).getMp() > 0) {
		...
		if (GuiCommandMenu.selected == GuiCommandMenu.DRIVE && GuiCommandMenu.submenu == GuiCommandMenu.SUB_DRIVE) {
			...
			} else if ((ExtendedPlayer.get(player).getDP() >= Constants
					.getCost(ExtendedPlayer.driveForms.get(GuiCommandMenu.driveselected)))
					|| ExtendedPlayer.get(player).getDP() > 0) {
	...
	public void commandBack() {
		if (GuiCommandMenu.submenu == GuiCommandMenu.SUB_MAIN)
			...
			Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX,
					Minecraft.getMinecraft().thePlayer.posY, Minecraft.getMinecraft().thePlayer.posZ,
					SoundHelper.Cancel, 1f, 1f, false);
		} else if (GuiCommandMenu.submenu == GuiCommandMenu.SUB_ITEMS) {
			GuiCommandMenu.submenu = GuiCommandMenu.SUB_MAIN;
			Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX,
					Minecraft.getMinecraft().thePlayer.posY, Minecraft.getMinecraft().thePlayer.posZ,
		...
		if (key != null)
			...
			case SUMMON_KEYBLADE:
				...
				if (props.inventoryKeychain.getStackInSlot(0) == null) {
					Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX,
							Minecraft.getMinecraft().thePlayer.posY, Minecraft.getMinecraft().thePlayer.posZ,
							SoundHelper.Error, 2f, 1f, false);
					break;
				}
				if (props.isKeybladeSummoned() == false && player.inventory.getCurrentItem() == null
						&& props.inventoryKeychain.getStackInSlot(0).getItem() instanceof ItemKeychain) {
					PacketDispatcher.sendToServer(new SummonKeyblade(
							((ItemKeychain) props.inventoryKeychain.getStackInSlot(0).getItem()).getKeyblade()));
					PacketDispatcher.sendToServer(new SyncExtendedPlayer(player));
				} else if (player.inventory.getCurrentItem() != null
						&& player.inventory.getCurrentItem().getItem() instanceof ItemKeyblade
						&& props.isKeybladeSummoned() == true) {
	...
	public void OnMouseWheelScroll(MouseEvent event) {
		...
		if (event.dwheel <= Constants.WHEEL_DOWN && KeyboardHelper.isScrollActivatorDown() && event.dwheel != 0) {
			...
			event.setCanceled(true);
			Minecraft.getMinecraft().theWorld.playSound(Minecraft.getMinecraft().thePlayer.posX,
					Minecraft.getMinecraft().thePlayer.posY, Minecraft.getMinecraft().thePlayer.posZ, SoundHelper.Move,
					1f, 1f, false);
		}
