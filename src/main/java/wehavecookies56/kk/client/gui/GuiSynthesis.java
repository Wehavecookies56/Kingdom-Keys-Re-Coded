...
public class GuiSynthesis extends GuiTooltip {
	...
	public final int MAIN = 0, BACK = 0, RECIPES = 1, FREEDEV = 2, MATERIALS = 3, CREATE = 4, TAKE1 = 5, TAKESTACK = 6,
			TAKEHALFSTACK = 7, TAKEALL = 8, DEPOSIT = 9;
	public int submenu;
	...
	public void initGui() {
		...
		this.buttonList.add(Back = new GuiButton(BACK, width - 105, height - ((height / 8) + 70 / 16), 100, 20,
				TextHelper.localize(Strings.Gui_Menu_Items_Button_Back)));
		this.buttonList.add(Create = new GuiButton(CREATE, 270, height - ((height / 8) + 70 / 16) - 25, 100, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Recipes_Create)));
		this.buttonList.add(Recipes = new GuiButton(RECIPES, 5, 65, 100, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Recipes)));
		this.buttonList.add(FreeDev = new GuiButton(FREEDEV, 5, 65 + 25, 100, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_FreeDev)));
		this.buttonList.add(Materials = new GuiButton(MATERIALS, 5, 90 + 25, 100, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Materials)));

		this.buttonList.add(Deposit = new GuiButton(DEPOSIT, 200, height - ((height / 8) + 70 / 16), 100, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Materials_Deposit)));

		this.buttonList.add(Take1 = new GuiButton(TAKE1, 195, height - ((height / 8) + 70 / 16) - 25, 75, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Materials_TakeOne)));
		this.buttonList.add(TakeHalfStack = new GuiButton(TAKEHALFSTACK, 270, height - ((height / 8) + 70 / 16) - 25,
				75, 20, TextHelper.localize(Strings.Gui_Synthesis_Main_Materials_TakeHalfStack)));
		this.buttonList.add(TakeStack = new GuiButton(TAKESTACK, 345, height - ((height / 8) + 70 / 16) - 25, 75, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Materials_TakeStack)));
		this.buttonList.add(TakeAll = new GuiButton(TAKEALL, 420, height - ((height / 8) + 70 / 16) - 25, 75, 20,
				TextHelper.localize(Strings.Gui_Synthesis_Main_Materials_TakeAll)));
	...
	protected boolean getInventoryMaterial(String material) {
		...
		for (ItemStack element : mc.thePlayer.inventory.mainInventory)
			if (element != null)
				if (element.getUnlocalizedName() == material)
					return true;
		return false;
	}
	...
	protected void actionPerformed(GuiButton button) {
		...
		case CREATE:
			if (isRecipeUsable(ExtendedPlayerRecipes.get(mc.thePlayer).knownRecipes.get(selected), 1))
				PacketDispatcher.sendToServer(new CreateFromSynthesisRecipe(
						ExtendedPlayerRecipes.get(mc.thePlayer).knownRecipes.get(selected), 1));
			break;
		case TAKE1:
			...
			if (foundMaterial || freeSlots >= 1)
				if (mats.getKnownMaterialsMap().get(materials.get(materialSelected)) >= 32)
					...
					PacketDispatcher.sendToServer(
							new TakeMaterials(mats.getKnownMaterialsMap().get(materials.get(materialSelected)),
									materials.get(materialSelected)));
			break;
		case TAKESTACK:
			materials.addAll(mats.getKnownMaterialsMap().keySet());
		...
		for (int i = 0; i < mats.getKnownMaterialsMap().size(); i++)
			if (materialSelected == i) {
				GL11.glPushMatrix();
				{
					...
					GL11.glScalef(2, 2, 2);
					drawString(fontRendererObj, TextHelper.localize(materials.get(i).toString() + ".name") + " x"
							+ mats.getKnownMaterialsMap().get(materials.get(i)), 0, 0, 0xFFF700);
				}
				GL11.glPopMatrix();
			...
			for (int i = 0; i < props.knownRecipes.size(); i++)
				if (selected == i) {
					GL11.glPushMatrix();
					{
						...
						GL11.glScalef(2, 2, 2);
						drawString(fontRendererObj, TextHelper.localize(props.knownRecipes.get(i).toString() + ".name"),
								0, 0, 0xFFF700);
					}
					GL11.glPopMatrix();

					drawString(fontRendererObj,
							TextHelper.localize(Strings.Gui_Synthesis_Main_Recipes_ReqMaterials) + TextHelper.ITALIC,
							posX, 100, 0x00C3FF);
					...
					while (it.hasNext()) {
						...
						GL11.glPushMatrix();
						{
							...
							if (synthMaterial == null) {
								...
								Minecraft.getMinecraft().getRenderItem()
										.renderItemAndEffectIntoGUI(pair.getKey().getItem(), 0, 0);
							} else {
						...
						drawString(fontRendererObj, material, (int) (posX + 18 + (materialLength * 1.05f)),
								114 + (distY * row), 0xFFFFFF);
						drawString(fontRendererObj, info,
								(int) (posX + 18 + fontRendererObj.getStringWidth(material) + (materialLength * 1.05f)),
								114 + (distY * row), colour);
						if (column == 1) {
							row++;
							column = 0;
							materialLength = 0;
						} else {
							materialLength = (fontRendererObj.getStringWidth(
									TextHelper.localize(ModItems.Chain_IncompleteKiblade.getUnlocalizedName() + ".name")
											+ " - You have XXXX"))
									+ 20/*
