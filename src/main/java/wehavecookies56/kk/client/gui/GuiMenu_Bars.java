...
public class GuiMenu_Bars extends GuiScreen {
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		if (drawPlayerInfo) {
			GL11.glPushMatrix();
			{
				drawCenteredString(fontRendererObj, mc.thePlayer.getDisplayNameString().toString(), this.width / 2,
						this.height / 2 + 80, 0xFFFFFF);
				drawString(fontRendererObj, "Lv: " + ExtendedPlayer.get(mc.thePlayer).getLevel(), this.width / 2 - 40,
						this.height / 2 + 90, 0xFFD900);
				drawString(fontRendererObj,
						"HP: " + (int) mc.thePlayer.getHealth() + "/" + (int) mc.thePlayer.getMaxHealth(),
						this.width / 2 - 40, this.height / 2 + 100, 0x00FF00);
				drawString(fontRendererObj,
						"MP: " + (int) ExtendedPlayer.get(mc.thePlayer).getMp() + "/"
								+ (int) ExtendedPlayer.get(mc.thePlayer).getMaxMp(),
						this.width / 2 - 40, this.height / 2 + 110, 0x4444FF);
