...
public class GuiJournal extends GuiScreen {
	...
	final int COLLAPSE_COMMANDMENU = 0, COLLAPSE_KEYBLADES = 1, COLLAPSE_BUGBLOX = 2, COLLAPSE_SYNTHESIS = 3,
			COLLAPSE_ARMOUR = 4, COLLAPSE_MAGIC = 5, COLLAPSE_DRIVEFORMS = 6, COLLAPSE_MISC = 7;

	final int TOPIC_COMMANDMENU_ABOUT = 200, TOPIC_COMMANDMENU_ATTACK = 201, TOPIC_COMMANDMENU_MAGIC = 202,
			TOPIC_COMMANDMENU_ITEMS = 203, TOPIC_COMMANDMENU_DRIVE = 204;
	final int TOPIC_KEYBLADES_ABOUT = 205, TOPIC_KEYBLADES_LIST = 206, TOPIC_KEYBLADES_SUMMONING = 207,
			TOPIC_KEYBLADES_CREATION = 208;
	...
	final int TOPIC_MISC_HEARTS = 224, TOPIC_MISC_MUNNY = 225, TOPIC_MISC_MENU = 226,
			TOPIC_MISC_CRAFTINGMATERIALS = 227, TOPIC_MISC_ORGANIZATIONWEAPONS = 228, TOPIC_MISC_MUSICDISCS = 229,
			TOPIC_MISC_COMMANDS = 230;

	final int SCROLL_COLLAPSE = 100, SCROLL_PAGE = 101;

	GuiButtonCollapse collapse_commandmenu, collapse_keyblades, collapse_bugblox, collapse_synthesis, collapse_armour,
			collapse_magic, collapse_driveforms, collapse_misc;

	GuiButtonScroll scroll_collapse, scroll_page;

	GuiButtonLink topic_commandmenu_about, topic_commandmenu_attack, topic_commandmenu_magic, topic_commandmenu_items,
			topic_commandmenu_drive, topic_keyblades_about, topic_keyblades_list, topic_keyblades_summoning,
			topic_keyblades_creation, topic_bugblox_about, topic_bugblox_standardblox, topic_bugblox_specialblox,
			topic_synthesis_about, topic_synthesis_materials, topic_synthesis_recipes, topic_armour_keyblade,
			topic_armour_organization, topic_magic_about, topic_magic_mp, topic_magic_magics, topic_driveforms_about,
			topic_driveforms_dp, topic_driveforms_forms, topic_misc_hearts, topic_misc_munny, topic_misc_menu,
			topic_misc_craftingmaterials, topic_misc_organizationweapons, topic_misc_musicdiscs, topic_misc_commands;
	...
	public GuiJournal() {
		...
		topic_bugblox_standardblox = new GuiButtonLink(TOPIC_BUGBLOX_STANDARDBLOX, 0, 0, 0, 0, "Standard Blox",
				0x0645AD);
		...
		topic_misc_craftingmaterials = new GuiButtonLink(TOPIC_MISC_CRAFTINGMATERIALS, 0, 0, 0, 0, "Crafting Materials",
				0x0645AD);
		topic_misc_organizationweapons = new GuiButtonLink(TOPIC_MISC_ORGANIZATIONWEAPONS, 0, 0, 0, 0,
				"Organization Weapons", 0x0645AD);
	...
	public void drawScreen(int mouseX, int mouseY, float partialTicks) {
		...
		drawTitle(collapse_commandmenu, xPos_coll_commandMenu, yPos_coll_commandMenu + (int) offset, 100, 12,
				"Command Menu", 1F);
		...
		drawTitle(collapse_keyblades, xPos_coll_keyblades, yPos_coll_keyblades + (int) offset, 100, 12, "Keyblades",
				1F);
		...
		drawTitle(collapse_synthesis, xPos_coll_synthesis, yPos_coll_synthesis + (int) offset, 100, 12, "Synthesis",
				1F);
		...
		drawTitle(collapse_driveforms, xPos_coll_driveforms, yPos_coll_driveforms + (int) offset, 100, 12,
				"Drive Forms", 1F);
	...
	public void drawTitle(GuiButtonCollapse button, int posX, int posY, int width, int height, String text,
			float scale) {
	...
	public void handleMouseInput() throws IOException {
		...
		if (Mouse.getEventDWheel() > 1 && Mouse.getEventDWheel() != 0)
			if (Mouse.getEventX() <= (xPos_coll_commandMenu * 2) + 200 + (scroll_collapse.width * 2)
					&& Mouse.getEventX() >= xPos_coll_commandMenu * 2)
				if (scrollPos > 0) {
					scrollPos -= 5;
					offset = (-scrollPos);
				} else if (scrollPos < 0) {
	...
	public static class GuiButtonCollapse extends GuiButton {
		...
		public void drawButton(Minecraft mc, int mouseX, int mouseY) {
			if (visible) {
				...
				int listHeight = (((res.getScaledHeight() / 4) + 7) + (res.getScaledHeight() / 2))
						- yPos_coll_commandMenu + 2;

				GL11.glScissor(xPos_coll_commandMenu, (distToBottomFromMisc) * 2 - 2, res.getScaledWidth(),
						listHeight * 2 - 1);
				GL11.glEnable(GL11.GL_SCISSOR_TEST);
